...
from humanoidverse.utils.motion_lib.torch_humanoid_batch_ehr03 import Humanoid_Batch
# from smpl_retarget.mink_retarget.retargeting.torch_humanoid_batch_ehr03 import Humanoid_Batch

def draw_joint_axis(viewer, mj_model, mj_data, joint_id, length=0.05):
    # 获取该关节的 body id（body 是关节挂载的刚体）
    body_id = mj_model.jnt_bodyid[joint_id]
    body_xpos = mj_data.xpos[body_id]  # 世界坐标系下的位置
    body_xmat = mj_data.xmat[body_id].reshape(3, 3)  # 世界坐标系下的旋转矩阵

    # 坐标轴可视化：X(红), Y(绿), Z(蓝)
    colors = [np.array([1, 0, 0, 1]), np.array([0, 1, 0, 1]), np.array([0, 0, 1, 1])]
    for i in range(3):
        start = body_xpos
        end = body_xpos + length * body_xmat[:, i]
        add_visual_capsule(viewer.user_scn, start, end, 0.005, colors[i])

    # 获取关节轴（在 joint frame 中），再变换到世界坐标
    axis_local = mj_model.jnt_axis[joint_id]
    joint_pos_local = mj_model.jnt_pos[joint_id]
    joint_pos_world = mj_data.xanchor[joint_id]  # 世界坐标系下的旋转中心

    axis_world = body_xmat @ axis_local
    end = joint_pos_world + length * axis_world

    # 可视化关节轴（黄色）
    add_visual_capsule(viewer.user_scn, joint_pos_world, end, 0.007, np.array([1, 1, 0, 1]))
...
def main(cfg : DictConfig) -> None:
    ...
    humanoid_xml = "./description/robots/ehr03/urdf/ehr03_simplifed.xml"
    print(humanoid_xml)
    ...
    if not vis_smpl:
        cfg_robot = OmegaConf.load("description/robots/ehr03/phc_ehr03.yaml")
    ...
    with mujoco.viewer.launch_passive(mj_model, mj_data, key_callback=key_call_back) as viewer:
        ...
        # print('\n'.join(f'{k:2d}  {name}'
        #                 for k, name in enumerate(humanoid_fk.body_names_augment[:28])))
        print("\n====== Rigid Body 名称列表（mujoco.model.body）======")
        for i in range(mj_model.nbody):
            print(f"{i:2d}  {mujoco.mj_id2name(mj_model, mujoco.mjtObj.mjOBJ_BODY, i)}")

        print("\n====== Joint 名称列表（mujoco.model.joint）======")
        for i in range(mj_model.njnt):
            print(f"{i:2d}  {mujoco.mj_id2name(mj_model, mujoco.mjtObj.mjOBJ_JOINT, i)}")
        ...
        while viewer.is_running():
            ...
            # for j in range(27):
            #     draw_joint_axis(viewer, mj_model, mj_data, j, length=0.08)
            if vis_contact: 
                viewer.user_scn.geoms[21].rgba = np.array([0, 1-curr_motion['contact_mask'][curr_time, 0], 0, 1])
                viewer.user_scn.geoms[27].rgba = np.array([0, 1-curr_motion['contact_mask'][curr_time, 1], 0, 1])
