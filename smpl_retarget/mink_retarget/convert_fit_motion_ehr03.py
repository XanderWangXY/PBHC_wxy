...
def count_pose_aa(motion):
    ...
    dof_axis = np.load('../description/robots/ehr03/dof_axis.npy', allow_pickle=True)
...
def main(
    ...
    elif robot_type in ["h1", "g1", "ehr03"]:
        assert (
            force_retarget
    ...
    for folder_name in folder_names:
        ...
        for filename in tqdm(files):
                ...
                if filename.suffix == ".npz" and "samp" not in str(filename):
                    ...
                    arr = np.load(filename)  # 根 + 24 关节 (XYZ)
                    mocap_fr = 30  # 与 .pkl 分支一致

                    # ---------- 提取根位移 & 关节欧拉角 ----------
                    root_trans = arr[:, 0, :]  # (F,3)
                    joint_eul = arr[:, 1:, :]  # (F,24,3) Euler-XYZ
                    ...
                    # ---------- Euler → Axis-Angle (pose_aa_full) ----------
                    pose_aa_full = (
                        sRot.from_euler("XYZ", joint_eul.reshape(-1, 3))
                        .as_rotvec()
                        .reshape(arr.shape[0], 24, 3)
                    )  # (F,24,3)

                    # 仅重排索引，不改名 —— 与原版 pose_aa_mj 一致
                    pose_aa_mj = pose_aa_full[:, smpl_2_mujoco]

                    # ---------- Axis-Angle → Quaternion ----------
                    pose_quat = (
                        sRot.from_rotvec(pose_aa_mj.reshape(-1, 3))
                        .as_quat()
                        .reshape(arr.shape[0], 24, 4)
                    )  # (F,24,4)

                    # ========== 以下保持与原版完全同步 ==========

                    # ① betas & gender --------------------------------------------------------
                    betas_np = np.zeros(10)  # TODO  ← 需填 betas，或改为实际读取
                    gender = "neutral"

                    origin_betas = torch.from_numpy(betas_np)
                    betas = torch.from_numpy(betas_np)

                    # ② 保存 motion_data（与原版字段一致） -------------------------------
                    motion_data = {
                        "pose_aa": pose_aa_full.reshape(arr.shape[0], -1),  # (F,72)
                        "trans": root_trans,
                        "beta": betas.numpy(),
                        "gender": gender,
                    ...
                    shape_new, scale = joblib.load("./mink_retarget/shape_optimized_neutral.pkl")
                    print("shape_new: ", shape_new)
                    scale *= 1.6
                    print("scale: ", scale)

                    with torch.no_grad():
                        # verts / joints for new shape
                        verts, joints = smpl_parser_n.get_joints_verts(
                            torch.from_numpy(motion_data["pose_aa"]),
                            shape_new,
                            torch.from_numpy(root_trans),
                        )
                        # verts / joints for origin shape —— 用于 foot_detect
                        origin_verts, origin_global_trans = smpl_parser_n.get_joints_verts(
                            torch.from_numpy(motion_data["pose_aa"]),
                            origin_betas.unsqueeze(0),
                            torch.from_numpy(root_trans),
                        )
                    ...
                    global_trans = joints[:, smpl_2_mujoco]  # (F,24,3)
                    ...
                    if upright_start:
                        B = pose_quat_global.shape[0]
                        pose_quat_global = (
                                sRot.from_quat(pose_quat_global.reshape(-1, 4))
                                * sRot.from_quat([0.5, 0.5, 0.5, 0.5]).inv()
                        ).as_quat().reshape(B, 24, 4)

                    # ⑤ 抽帧到 30 fps --------------------------------------------------------
                    skip = int(mocap_fr // 30)
                    fps = 30
                    global_trans_skip = global_trans[::skip]
                    pose_quat_skip = pose_quat_global[::skip]

                    # ⑥ 脚底接触 & 轨迹校正 ---------------------------------------------------
                    feet_l, feet_r = foot_detect(origin_global_trans[::skip])
                    contact_mask = np.concatenate([feet_l, feet_r], axis=-1)
                    if correct:
                        global_trans_skip = correct_motion(contact_mask, origin_verts[::skip], global_trans_skip)

                    # ⑦ Retarget --------------------------------------------------------------
                    from retargeting.mink_retarget import retarget_fit_motion
                    ...
                    motion_data = {k: np.asarray(v) for k, v in motion_data.items()}
                    motion_data["contact_mask"] = contact_mask
                    pose_aa_flat, dof = count_pose_aa(motion_data)
                    motion_data["pose_aa"] = pose_aa_flat
                    motion_data["dof"] = dof
                    import re
                    output_folder_path = "./retargeted_motion_data/mink"
                    os.makedirs(output_folder_path, exist_ok=True)
                    safe_stem = re.sub(r"[^a-zA-Z0-9]", "_", filename.stem)
                    path = os.path.join(output_folder_path, f"{safe_stem}.pkl")
                    print(path)
                    with open(path, "wb") as f:
                        pickle.dump({filename: motion_data}, f, protocol=pickle.HIGHEST_PROTOCOL)

                    if robot_type in ["h1", "g1", "ehr03"]:
