...
####################################################################################
        ...
        idx_dof = torch.tensor([src_dof_order.index(n) for n in tgt_dof_order], device=dof_pos.device)
        dof_pos = dof_pos.index_select(1, idx_dof)
        dof_vel = dof_vel.index_select(1, idx_dof)
        ...
        idx_body28 = torch.tensor([src_body_order.index(n) for n in tgt_body_order], device=rg_pos.device)

        # -- 28 维张量直接重排
        rg_pos = rg_pos.index_select(1, idx_body28)
        rb_rot = rb_rot.index_select(1, idx_body28)
        body_vel = body_vel.index_select(1, idx_body28)
        body_ang_vel = body_ang_vel.index_select(1, idx_body28)

        # -- 若带 “_t” 版本（31 维 = 28 + 3 额外 link），只调前 28，后 3 保持不动
        if "rg_pos_t" in locals():
            pad = torch.arange(28, rg_pos_t.shape[1], device=rg_pos_t.device)  # = [28,29,30]
            idx_body31 = torch.cat([idx_body28, pad], dim=0)

            rg_pos_t = rg_pos_t.index_select(1, idx_body31)
            rg_rot_t = rg_rot_t.index_select(1, idx_body31)
            body_vel_t = body_vel_t.index_select(1, idx_body31)
            body_ang_vel_t = body_ang_vel_t.index_select(1, idx_body31)
        # ------------------------------------------------------------
