...
URCIRobotTypeEhr03 = TypeVar('URCIRobotType', bound='URCIRobot')
ObsCfgEhr03 = Union[DictConfig, Callable[[URCIRobotTypeEhr03],np.ndarray]]
URCIPolicyObsEhr03 = Tuple[ObsCfgEhr03, Callable]

CfgTypeEhr03 = Union[OmegaConf, ListConfig, DictConfig]
...
class URCIRobotEhr03:
    ...
    def __init__(self, cfg: CfgTypeEhr03):
    ...
    def routing(self, cfg_policies: List[URCIPolicyObsEhr03]):
        """
    ...
    def SetObsCfg(self, obs_cfg: ObsCfgEhr03):
    ...
    def KickMotionLib(self):
        ...
        relyaw_heading_inv_quat_expand = relyaw_heading_inv_quat.unsqueeze(1).expand(-1, 31, -1).reshape(-1, 4)
        ...
        heading_inv_rot_expand = heading_inv_rot.unsqueeze(1).expand(-1, 31, -1).reshape(-1, 4)
    ...
    def _check_init(self):
        ...
        assert self.num_dofs == 27, "In policy level, only 27 dofs are supported for now"
    ...
    def _make_init_pose(self):
        ...
        assert self.num_dofs == 27, "Only 27 dofs are supported for now"
    ...
    def _make_buffer(self):
        ...
        self._obs_global_ref_body_vel = torch.zeros(31*3, dtype=torch.float32)  # 31 rigid bodies, each has 3 velocity components
        self._obs_local_ref_rigid_body_vel = torch.zeros(31*3, dtype=torch.float32)
        self._obs_local_ref_rigid_body_pos_relyaw = torch.zeros(31*3, dtype=torch.float32)
        ...
        
    def _make_motionlib(self, cfg_policies: List[URCIPolicyObsEhr03]):
    ...
    def _make_init_save_motion(self):
        ...
        self._dof_axis = np.load('description/robots/ehr03/dof_axis.npy', allow_pickle=True)
