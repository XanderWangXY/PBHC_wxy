...
class Humanoid_Batch:
    ...
    def __init__(self, cfg, device=torch.device("cpu")):
        ...
        def _strip_ns(root):
            """把 {namespace}tag → tag，便于 find()/xpath() 普通匹配"""
            for elem in root.iter():
                if isinstance(elem.tag, str) and elem.tag.startswith("{"):
                    elem.tag = elem.tag.split("}", 1)[-1]

        _strip_ns(tree.getroot())          # ← 关键：去掉 namespace
        # --------------------------------------------------

        # ---------- worldbody & joints（只在 worldbody 里递归找） ----------
        worldbody = tree.getroot().find("worldbody")
        joint_elems = worldbody.findall(".//joint") if worldbody is not None else []
        # ① 只收集带 name 的关节，跳过匿名 joint
        joints = sorted([j.attrib["name"] for j in joint_elems if "name" in j.attrib])
        # ------------------------------------------------------------------

        # ---------- actuator / motor ----------
        actuator_elem = tree.getroot().find("actuator")  # 可能为 None
        motor_elems = actuator_elem.findall("*") if actuator_elem is not None else []

        motors = sorted([a.attrib["name"] for a in motor_elems if "name" in a.attrib])

        if len(motors) == 0:
            raise ValueError("No named <motor> (or similar) found under <actuator> in the MJCF file")
        # --------------------------------------

        self.dof_axis = []                       # ↓ 后面统一用 joint_elems
        # ============== 其余原代码保持不动 =================
        ...
        # ---------- 选取需要的 DOF 关节 ----------
        self.has_freejoint = True  # 你的模型根本就有自由基

        scan_iter = []
        for j in joint_elems:
            # ① 没有 name → 视作根平移 / 附带虚关节，直接跳过
            if "name" not in j.attrib:
                continue
            # ② 有 name → 真正的可控关节
            scan_iter.append(j)
        ...
        for j in scan_iter:
            name = j.attrib["name"]
            if "axis" in j.attrib:
                axis = [int(x) for x in j.attrib["axis"].split()]
                self.dof_axis.append(axis)
                selected.append((name, axis))
            else:
                skipped.append(name)

        # ---------- 调试输出 ----------
        print("=== DOF 统计 ===")
        print(f"  计入 DOF 的关节数: {len(selected)}")
        for n, ax in selected:
            print(f"    ✓ {n:20s} axis={ax}")
        if skipped:
            print(f"  跳过(无 axis) 的关节数: {len(skipped)}")
            for n in skipped:
                print(f"    ✗ {n}")
        print("================\n")
        # --------------------------------
        ...
        # ===== 下面逻辑（extend_config、映射、mesh 等）完全保持 =====
        ...
        self.extend_to_proper_mapping = [
            ...
            for name in self._proper_kinematic_structure
        ]
        self.proper_to_extend_mapping = [
            self._proper_kinematic_structure.index(name)
            for name in self.body_names_augment
        ]
