...
class LeggedRobotMotionTracking(LeggedRobotBase):
    ...
    def _draw_debug_vis(self):
        """
        可视化调试：关节球 + 速度箭头
        兼容旧版 Isaac Gym（无 Quat.from_matrix）。
        """
        ...
        # ---------- 工具：旋转矩阵 → gymapi.Quat ----------
        def rot_mat_to_quat(m: np.ndarray) -> gymapi.Quat:
            """将 3×3 NumPy 旋转矩阵转为 gymapi.Quat(x, y, z, w)"""
            if hasattr(gymapi.Quat, "from_matrix"):  # 新版接口
                return gymapi.Quat.from_matrix(m)
        ...
        for env_id in range(1):  # 如需全部环境，改为 range(self.num_envs)
            if not self.config.use_teleop_control:
                # ---------- 画 marker & 刚体球 ----------
                for pos_id, pos_joint in enumerate(self.marker_coords[env_id]):
                    color = (self.config.robot.motion.visualization.marker_joint_colors[pos_id %
                                                                                        len(self.config.robot.motion.visualization.marker_joint_colors)]
                             if self.config.robot.motion.visualization.customize_color else (0.3, 0.3, 0.3))
                    self.simulator.draw_sphere(pos_joint, 0.04, tuple(color), env_id, pos_id)
                #
                for pos_id, pos_joint in enumerate(self._rigid_body_pos_extend[env_id]):
                    color = (self.config.robot.motion.visualization.marker_joint_colors[pos_id %
                                                                                        len(self.config.robot.motion.visualization.marker_joint_colors)]
                             if self.config.robot.motion.visualization.customize_color else (0.3, 0.3, 0.3))
                    self.simulator.draw_sphere(pos_joint, 0.04, tuple(color), env_id, pos_id)

                # for pos_id, pos_joint in enumerate(self.ref_joint_pos[env_id]):
                #     color = (self.config.robot.motion.visualization.marker_joint_colors[pos_id %
                #                                                                         len(self.config.robot.motion.visualization.marker_joint_colors)]
                #              if self.config.robot.motion.visualization.customize_color else (0.3, 0.3, 0.3))
                #     self.simulator.draw_sphere(pos_joint, 0.04, tuple(color), env_id, pos_id)
                ...
                # # ---------- 画速度箭头 ----------
                # ref_vels = self.ref_body_vel_extend[env_id]
                # sim_vels = self._rigid_body_vel_extend[env_id]
                #
                # for pos_id, (ref_pos, ref_vel, sim_pos, sim_vel) in enumerate(zip(
                #         self.marker_coords[env_id], ref_vels,
                #         self._rigid_body_pos_extend[env_id], sim_vels)):
                #
                #     color = tuple(self.config.robot.motion.visualization.marker_joint_colors[
                #                       pos_id % len(self.config.robot.motion.visualization.marker_joint_colors)])
                #
                #     def draw_velocity_arrow_line(start_pos: torch.Tensor,
                ...
                #             return
                #
                #         direction = velocity / vel_norm  # GPU tensor
                #         arrow_len = (scale * vel_norm).item()
                #
                #         # GPU 上先算好中心位置，再转 CPU
                #         arrow_center = (start_pos + 0.5 * arrow_len * direction).cpu().numpy()
                #         axis = direction.cpu().numpy()
                #
                #         # 生成正交基向量
                ...
                #         rot_mat = np.stack([right, up, axis], axis=1)
                #
                #         quat = rot_mat_to_quat(rot_mat)
                #
                #         tf = gymapi.Transform()
                #         tf.p = gymapi.Vec3(*arrow_center.tolist())
                #         tf.r = quat
                #
                #         arrow_geom = gymutil.WireframeBoxGeometry(
                #             arrow_len, 0.005, 0.005, None, color=color)
                #         gymutil.draw_lines(
                #             arrow_geom, self.simulator.gym, self.simulator.viewer,
                #             self.simulator.envs[env_id], tf)
                #
                #     # 参考 & 仿真速度箭头
                #     # draw_velocity_arrow_line(ref_pos, ref_vel)
                #     # draw_velocity_arrow_line(sim_pos, sim_vel)
                #     # ① 工具：tuple → Vec3
                #     # ---------- 角速度柱状条 ----------
                #     z_axis = torch.tensor([0, 0, 1.0], device=self.device)
                #     vel_scale = 0.05  # 1 rad/s => 5 cm
                #     vel_clip = 20.0  # 上下限
                #     thickness = 0.02  # 柱体粗细
                #
                #     dof_vel_ref = self.ref_joint_vel  # [B, D]
                #     dof_vel_sim = self.simulator.dof_vel  # [B, D]
                #
                #     def darker(color, k=0.4):
                #         return tuple(float(c) * k for c in color)
                #
                #     for j in range(dof_vel_ref.shape[1]):
                #         base_color = tuple(self.config.robot.motion.visualization.marker_joint_colors[
                #                                j % len(self.config.robot.motion.visualization.marker_joint_colors)])
                #
                #         # ---------- 参考柱 ----------
                #         ref_len = torch.clip(dof_vel_ref[env_id, j], -vel_clip, vel_clip) * vel_scale
                #         height = abs(ref_len.item())
                #         if height > 1e-4:
                #             center = (self.marker_coords[env_id, j] + 0.5 * ref_len * z_axis).cpu().numpy()
                #             geom = gymutil.WireframeBoxGeometry(
                #                 thickness, thickness, height, None, color=base_color)
                #             tf = gymapi.Transform(p=gymapi.Vec3(*center))
                #             gymutil.draw_lines(geom, self.simulator.gym, self.simulator.viewer,
                #                                self.simulator.envs[env_id], tf)
                #
                #         # ---------- 仿真柱 ----------
                #         sim_len = torch.clip(dof_vel_sim[env_id, j], -vel_clip, vel_clip) * vel_scale
                #         height = abs(sim_len.item())
                #         if height > 1e-4:
                #             center = (self._rigid_body_pos_extend[env_id, j] + 0.5 * sim_len * z_axis).cpu().numpy()
                #             geom = gymutil.WireframeBoxGeometry(
                #                 thickness, thickness, height, None, color=darker(base_color))
                #             tf = gymapi.Transform(p=gymapi.Vec3(*center))
                #             gymutil.draw_lines(geom, self.simulator.gym, self.simulator.viewer,
                #                                self.simulator.envs[env_id], tf)
                ...
                for pos_id, pos_joint in enumerate(self.teleop_marker_coords[env_id]):
                    self.simulator.draw_sphere(pos_joint, 0.04,
                                               (0.851, 0.144, 0.07), env_id, pos_id)
    ...
    def _pre_compute_observations_callback(self):
        ...
        # self._debug_pos_mismatch()
        # self._debug_torque_print()

    def _debug_torque_print(self, frame_interval: int = 5):
        """
        每隔 frame_interval 步打印一次各关节的力矩（torque），
        并列出力矩绝对值最大的前 10 个关节。
        """
        if self.common_step_counter % frame_interval != 0:
            return

        # 假设 torques 是 shape [B, D]
        B0 = 0
        torques = self.torques[B0]  # [D]
        abs_torque = torques.abs()

        print(f"[Step {self.common_step_counter}] torque summary:")
        print(f"mean={abs_torque.mean():.3f}, max={abs_torque.max():.3f}")

        # 获取前 10 个最大力矩的索引
        topk = torch.topk(abs_torque, 10)

        print("Top-10 joint torque (N·m):")
        print("Idx  Name                 |  Torque")
        for idx, val in zip(topk.indices, topk.values):
            name = (self.simulator.dof_names[idx]
                    if hasattr(self.simulator, 'dof_names') else str(idx.item()))
            t_val = torques[idx].item()
            print(f"{idx:>2d} {name:<20s} {t_val:+8.3f}")

    def _debug_pos_mismatch(self, frame_interval: int = 5):
        """
        每隔 frame_interval 步打印一次关节“位置”误差（弧度），
        并列出误差最大的前 10 个关节的 ref / sim 数值。
        """
        if self.common_step_counter % frame_interval != 0:
            return

        # ① 关节角度 L2 误差
        joint_err = self.dif_joint_angles.norm(dim=-1)  # [B]
        print(f"[Step {self.common_step_counter}] "
              f"joint_pos_err  mean={joint_err.mean():.3f}, "
              f"max={joint_err.max():.3f}")

        # ② 躯干姿态（四元数）误差 ‒ 用四元数差的角度
        q_ref = self.ref_body_rot_extend[:, 0]  # torso idx=0
        q_sim = self._rigid_body_rot_extend[:, 0]
        dot = torch.sum(q_ref * q_sim, dim=-1).clamp(-1., 1.)
        ang_e = 2 * torch.acos(dot.abs())  # ∈[0,π]
        print(f"root_rot_err    mean={ang_e.mean():.3f} rad, "
              f"max={ang_e.max():.3f} rad")

        # -----------------------------------------------
        # ③ 列出误差最大的关节，并显示 ref / sim 具体角度
        B0 = 0  # 只看 env-0
        err_each = self.dif_joint_angles.abs()[B0]  # |ref-sim|
        ref_joint = self.ref_joint_pos[B0]  # 参考角度
        sim_joint = self.simulator.dof_pos[B0]  # 仿真角度

        topk = torch.topk(err_each, 15)
        print("Top-10 joint-pos error (rad):")
        print("Idx  Name                 |   Ref   |   Sim   |  |Δ|")
        for idx, val in zip(topk.indices, topk.values):
            name = (self.simulator.dof_names[idx]
                    if hasattr(self.simulator, 'dof_names') else str(idx.item()))
            ref_v = ref_joint[idx].item()
            sim_v = sim_joint[idx].item()
            print(f"{idx:>2d} {name:<20s} {ref_v:+7.3f}  {sim_v:+7.3f}  {val.item():7.3f}")
        # -----------------------------------------------
